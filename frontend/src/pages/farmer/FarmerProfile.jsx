...
export default function FarmerProfilePage() {
  ...
  return (
    <PageShell title="Your Farmer Profile">
      <motion.div
      ...
      >
        ...
        >
          ...
          <div className="relative flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
            ...
            <div className="flex-1">
              <h1 className="text-3xl font-black text-accent mb-2 font-display">{user?.first_name || 'Farmer'}</h1>
              ...
              <div className="mt-6 flex flex-wrap justify-center md:justify-start gap-3">
                ...
                <span className="px-4 py-1.5 rounded-full bg-emerald-100 text-emerald-700 font-bold text-xs border border-emerald-200">
                  Active Vendor
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <motion.div variants={itemVariants} className="space-y-4">
            <SectionHeader icon={Building} title="Farm Information" />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <LabelValue icon={Building} label="Farm Name" value={profile.farm_name} />
              <LabelValue icon={MapPin} label="Village" value={profile.village} />
              <LabelValue icon={MapPin} label="Taluka" value={profile.taluka} />
              <LabelValue icon={MapPin} label="City / District" value={profile.city} />
          ...
          <motion.div variants={itemVariants} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <LabelValue icon={Briefcase} label="Aadhaar Number" value={user?.aadhaar_number} />
              </div>
              <div className="sm:col-span-2 flex flex-col gap-3 p-4 rounded-xl bg-white/40 backdrop-blur-sm border border-white/20 shadow-sm">
                <div className="flex items-center gap-2">
                  <FileCheck className="h-4 w-4 text-accent" />
                  <span className="text-xs font-medium text-text-muted uppercase">Farmer Certificates</span>
                </div>
                {profile.certificates?.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {profile.certificates.map((cert, idx) => (
                      <span key={idx} className="px-3 py-1 bg-accent/10 text-accent text-[10px] font-bold rounded-lg border border-accent/20">
                        {cert}
                      </span>
                    ))}
                  </div>
                ) : (
                  <span className="text-sm italic text-text-muted">No certificates uploaded</span>
                )}
              </div>
            </div>
          </motion.div>
