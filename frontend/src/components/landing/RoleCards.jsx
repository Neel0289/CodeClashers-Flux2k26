...
const roles = [
  ...
  { icon: Store, name: 'Buyer', bg: 'bg-brown-surface', points: ['Browse produce', 'Offer prices', 'Pay escrow'] },
        ...
        {roles.map((role) => (
          <motion.div key={role.name} whileHover={{ y: -6 }}>
            <Card className={`${role.bg} transition hover:shadow-glow`}>
              <role.icon className="mb-3 text-accent-bright" />
              <h3 className="mb-2 text-xl">{role.name}</h3>
              {role.points.map((point) => <p key={point} className="text-sm text-text-muted">- {point}</p>)}
            </Card>
          </motion.div>
        ))}
      </motion.div>
